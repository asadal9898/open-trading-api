...
def _moderate_accounts(pf, mode: str) -> list:
    ...
    계좌체계 재설계 2-3b: moderate_order_runner.py(D) 2-2b 와 동일 패턴으로 전환 —
...
def main():
    ...
    today = date.today()
    pf = load_portfolio()
    # ⚠️ 계좌체계 재설계 2-3b: accounts_of 기반 _moderate_accounts 는 AccountInfo 객체를
    ...
    accounts_raw = _moderate_accounts(pf, mode)
    accounts = [(u, info.legacy_name) for u, info in accounts_raw]
    ...
    # trading_active — 2-3b: info.trading_active(새 API, AccountInfo) 로 직접 조회.
    # raw YAML 재파싱(_trading_active/alloc_raw) 제거 — D 2-2b 와 동일.
    active_accounts = [(u, info.legacy_name) for u, info in accounts_raw if info.trading_active]
    try:
