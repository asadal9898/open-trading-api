...
for d in alldays:
    ...
    for code in list(positions.keys()):
        ...
        gain=(p/avg-1)*100
        # 물타기
        if gain<=AVGDOWN and not pos["added"] and cash>=UNIT:
            pos["units"]+=UNIT/p; pos["cost"]+=UNIT; pos["added"]=True; cash-=UNIT
    ...
    for code in codes:
        ...
        if (p/lo-1)*100<=BUY_ZONE and is_buyable_phase(phase_at(code,d)):
            ...
            positions[code]={"units":UNIT/p,"cost":UNIT,"entry":d,"added":False}
            cash-=UNIT
    # 연말 스냅샷
    yearly[d.year]=total_value(d)
    _stats.append((d, len(positions), cash, total_value(d)))
...
print()
print("##### 자금 활용률 #####")
ratios=[c/v*100 for _,_,c,v in _stats if v>0]
slots=[n for _,n,_,_ in _stats]
print(f"  평균 현금비중: {_st.mean(ratios):.1f}%   중앙값 {_st.median(ratios):.1f}%")
print(f"  평균 보유종목: {_st.mean(slots):.1f}개   중앙값 {_st.median(slots):.0f}개 (최대 10)")
from collections import Counter
c=Counter(slots)
print("  보유종목수 분포:")
for k in sorted(c):
    pct=c[k]/len(slots)*100
    bar="█"*int(pct/2)
    print(f"    {k:2d}종목: {pct:5.1f}% {bar}")
