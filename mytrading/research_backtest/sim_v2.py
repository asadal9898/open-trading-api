...
PREMIUM = 0.0
CAPITAL = int(os.environ.get("CAPITAL", "10000000"))       # 1000만원
CASH_FLOOR = float(os.environ.get("CASH_FLOOR", "0")) / 100   # 현금 하한 비율
...
def slots_unit(v):
    ...
    inv = v * (1.0 - CASH_FLOOR)          # 현금 하한을 뺀 투자 가능액
    s = max(1, min(int(inv // MIN_UNIT), MAX_SLOTS))
    return s, inv / s
